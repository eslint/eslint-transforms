...
module.exports = function(fileInfo, api) {
    ...
    root.find(j.MemberExpression, {
        ...
        if (method === "getComments") {
            ...
            console.warn(
                formatBoldYellow(
                    `${path.relative(process.cwd(), fileInfo.path)}:${node.loc.start.line}:${node.loc.start.column} The "getComments()" method has been removed. Please use "getCommentsBefore()", "getCommentsAfter()", or "getCommentsInside()" instead. https://eslint.org/docs/latest/use/migrate-to-9.0.0#-removed-sourcecodegetcomments`
                )
            );
            return node;
        }
...
module.exports.formatBoldYellow = formatBoldYellow;
