...
module.exports = function(fileInfo, api) {
    ...
    root.find(j.CallExpression, {
        ...
        // Replace all instances of context.getSourceCode() with sourceCode variable
        if (method === "getSourceCode") {
            return j.identifier("sourceCode");
        }
    ...
    // Add a variable declaration for `const sourceCode = context.sourceCode ?? context.getSourceCode()` immediately inside the create() method
    root.find(j.Property, {
        key: { name: "create" }
    }).replaceWith(({ node }) => {
        const sourceCodeDeclaration = j.variableDeclaration("const", [
        ...
        node.value.body.body.unshift(sourceCodeDeclaration);
